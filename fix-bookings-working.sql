-- FINAL WORKING fix for bookings table
...
-- Cast dates properly and calculate months (minimum 1 month)
UPDATE bookings
SET total_months = GREATEST(1, 
  CEIL(
    EXTRACT(EPOCH FROM (end_date::timestamp - start_date::timestamp)) / (30.0 * 24 * 60 * 60)
  )::INTEGER
)
